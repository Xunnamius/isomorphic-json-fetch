...
import { fetch, setGlobalFetchConfig } from '../src/index';
...
const useSwr = async (key: string, fetcher: (url: string) => Promise<unknown>) => {
  ...
  try {
    data = (await fetcher(key)) as typeof data;
  } catch (e) {
    error = e;
  }
...
const clientFactory = async (
  ex: (
    doSomethingWith: AnyFunction,
    handleErr: AnyFunction,
    handleException: AnyFunction
  ) => (params: { url: string }) => Promise<void>,
  o: number[][]
) => {
...
describe('isomorphic-json-fetch [README EXAMPLES]', () => {
  ...
  test('example 1-2 works', async () => {
    ...
    await clientFactory(
      (doSomethingWith, handleErr, handleException) => async ({ url: URL }) => {
        try {
          const { res, json } = await fetch.get<{ myData: number }>(URL);

          if (!json)
            return void handleErr(`response code outside 200-299: ${res.status}`);
  ...
  test('example 1-3 works', async () => {
    ...
    await clientFactory(
      (doSomethingWith, handleErr, handleException) => async ({ url: URL }) => {
        const configuration = {
          // <== this can also be set globally, see below
          method: 'POST',
          body: { query: 'some-string' }
        };

        try {
          const { json, error } = await fetch<{ myData: number }, { message: string }>(
            URL,
            configuration
          );
  ...
  test('example 1-4 works', async () => {
    ...
    await clientFactory(
      (doSomethingWith, _, handleException) => async ({ url: URL }) => {
        try {
          doSomethingWith(
            (await fetch.post<{ myData: number }>(URL, { rejects: true })).json?.myData
          );
        } catch (e) {
  ...
  test('example 1-5 works', async () => {
    ...
    await clientFactory(
      (doSomethingWith, handleErr) => async ({ url: URL }) => {
  ...
  test('example 2 works', async () => {
    ...
    await unitServer({
      client: async ({ url: URL }) => {
        ...
        const { json: j } = await fetch.get<'technically valid JSON', { error: string }>(
          URL
        );
