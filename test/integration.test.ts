...
describe('isomorphic-json-fetch [INTEGRATION TESTS: CJS]', () => {
    ...
    it('should allow getting and setting global config', async () => {
      ...
      await unitServerPuppeteer({
        client: async ({ page, run }) => {
          ...
          expect(
            await page.evaluate(() => (window as any).getGlobalFetchConfig())
          ).toStrictEqual(getExpectedConfig());
    ...
    it('should fetch properly', async () => {
      ...
      await unitServerPuppeteer({
        client: async ({ page, run, url }) => {
          ...
          expect(
            await page.evaluate(
              async (url) => (await (window as any).fetch(url)).json,
              url
            )
          ).toStrictEqual({ method: 'POST' });
          expect(
            await page.evaluate(
              async (url) => (await (window as any).fetch.get(url)).json,
              url
            )
          ).toStrictEqual({ method: 'GET' });
        }
      });
    });
  });
  ...
  describe('server environment', () => {
    ...
    it('should allow getting and setting global config', async () => {
      ...
      const { getGlobalFetchConfig, setGlobalFetchConfig } = await import(
        `${__dirname}/../${main}.js`
      );
