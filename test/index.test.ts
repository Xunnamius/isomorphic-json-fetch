import { fetch, getGlobalFetchConfig, setGlobalFetchConfig } from '../src/index';
  ...
  describe('::fetch', () => {
    ...
    it("rejects with FetchError if non-2xx & rejects=true; doesn't if rejects=false", async () => {
      ...
      await unitServer({
        code: 404,
        client: async ({ url }) =>
          expect((await fetch(url)).error).toStrictEqual({ method: 'POST' })
      });

      await unitServer({
        code: 404,
        client: async ({ url }) => {
          expect((await fetch(url, { rejects: false })).error).toStrictEqual({
            method: 'POST'
          });
        }
      });
